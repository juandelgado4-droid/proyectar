...
async function handleMediaUpdate(data) {
  ...
  if (songKey !== lastSongKey && rawTitle) {
    ...
    if (cached) {
      ...
      if (cached.manualSynced && LrcParser.hasSyncedTimestamps(cached.manualSynced)) {
      ...
      if (cached.synced && LrcParser.hasSyncedTimestamps(cached.synced)) {
    ...
    await setCache(ca, ct, null, result.synced);
...
async function fetchAllLyrics(artist, title, durationMs = null) {
  let triedTimedScraper = false;
  if (!triedTimedScraper && window.electronAPI && window.electronAPI.scrapeLyrics) {
    triedTimedScraper = true;
    console.log(`Buscando letra sincronizada en Lyricsify para: ${artist} - ${title}`);
    const lyricsifySynced = await window.electronAPI.scrapeLyrics(artist, title);
    if (hasSyncedTimestamps(lyricsifySynced)) {
      console.log('Letra sincronizada encontrada en Lyricsify');
      return { plain: null, synced: lyricsifySynced };
    }
  }

  // 1. lrclib.net � best source, has synced lyrics
  const lrcResult = await fetchLrclib(artist, title, durationMs);
  if (hasSyncedTimestamps(lrcResult.synced)) return { plain: null, synced: lrcResult.synced };

  // 1.5. Hidden Scraper Fallback (lyricsify, syair) for synced lyrics
  if (!triedTimedScraper && window.electronAPI && window.electronAPI.scrapeLyrics) {
    console.log(`Intentando scraper inteligente para: ${artist} - ${title}`);
    const scrapedSynced = await window.electronAPI.scrapeLyrics(artist, title);
    if (hasSyncedTimestamps(scrapedSynced)) {
      console.log('¡Letra sincronizada encontrada por scraper!');
      return { plain: null, synced: scrapedSynced };
    }
  }

  // 2. lyrics.ovh � plain lyrics fallback
  // 3. Try with cleaned title variations
  const altTitle = title.replace(/[''´`]/g, "'");
  if (altTitle !== title) {
    const alt = await fetchLrclib(artist, altTitle, durationMs);
    if (hasSyncedTimestamps(alt.synced)) return { plain: null, synced: alt.synced };
  }
  ...
  try {
    ...
    for (const track of limited) {
      ...
      if (!existing) {
        ...
        await setCache(ca, ct, null, result.synced || null);
      }
    }
  } catch {}
}
